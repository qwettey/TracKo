...
import { TrendingUp, TrendingDown, Minus, Plus, Ship } from 'lucide-react';
import { FreightRecord } from '../types';

interface Props {
    history: FreightRecord[];
    onUpdateHistory: (records: FreightRecord[]) => void;
}

const FreightTracking: React.FC<Props> = ({ history, onUpdateHistory }) => {
    ...
    const statusIcon = (status: FreightRecord['status']) => {
    ...
    const statusColor = (status: FreightRecord['status']) => {
    ...
    return (
        <div className="animate-in fade-in duration-500">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h2 className="text-3xl font-bold text-slate-800">Navlun Takibi</h2>
                    <p className="text-slate-500 mt-1">Konteyner navlun fiyatı geçmişi.</p>
                </div>
                <button
            ...
            {latest && (
                <div className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-2xl p-6 mb-6 flex items-center gap-4">
                    <div className="bg-white/20 p-3 rounded-xl"><Ship size={24} /></div>
                    <div>
                        <p className="text-emerald-100 text-sm">Güncel Navlun Fiyatı</p>
                        <p className="text-3xl font-bold">${latest.price.toLocaleString()}</p>
                        <p className="text-emerald-200 text-xs mt-0.5">{latest.date} tarihli</p>
                    </div>
                </div>
            )}
            ...
            {showForm && (
                <div className="bg-white border border-slate-200 rounded-2xl p-6 mb-6 shadow-sm">
                    <h3 className="text-lg font-semibold text-slate-700 mb-4">Yeni Navlun Kaydı</h3>
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-slate-50 border-b border-slate-200">
                            ...
                            <th className="text-right px-6 py-3 font-semibold text-slate-600">Fiyat ($)</th>
                            <th className="text-right px-6 py-3 font-semibold text-slate-600">Değişim</th>
                        </tr>
                    </thead>
                    <tbody>
                        {history.length === 0 && (
                            <tr><td colSpan={3} className="text-center py-12 text-slate-400">Henüz kayıt yok.</td></tr>
                        )}
                        {history.map(r => (
                            <tr key={r.id} className="border-b border-slate-100 hover:bg-slate-50">
                                <td className="px-6 py-4 text-slate-700">{r.date}</td>
                                <td className="px-6 py-4 text-right font-semibold text-slate-800">${r.price.toLocaleString()}</td>
                                <td className="px-6 py-4 text-right">
                                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-semibold ${statusColor(r.status)}`}>
                                        {statusIcon(r.status)}{r.change}
                                    </span>
                                </td>
                            </tr>
                        ))}
